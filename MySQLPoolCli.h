...
class shm_data_t {
    public:
        shm_data_t(size_t);
        vector<vector<string> > read_data(int, shm_log_t *);
        char *get_shm_ptr();
        ~shm_data_t();
    private:
        char *shm_ptr;
...
shm_data_t::shm_data_t(size_t sz) {
    key_t mykey;
...
vector<vector<string> > shm_data_t::read_data(int data_id, shm_log_t *shm_log) {
    int i,j,ib,jb;
    char *trav, *prev, *nxtptr, *ptr, *target;
    int nodeCnt = 0;
...
/* Set up some constants for the segment */
    char * endOfSeg = this->shm_ptr + *((size_t *) this->shm_ptr);
    char * nextResInt = this->shm_ptr + sizeof(size_t);
    char * firstResPtr = this->shm_ptr + sizeof(size_t) + sizeof(int);
    char * startOfData = this->shm_ptr + sizeof(size_t) + sizeof(int) + sizeof(size_t);
/*     */

    chrstr[1] = '\0';
    assert(this->shm_ptr + *((size_t *) firstResPtr) < endOfSeg); 
    assert(this->shm_ptr + *((size_t *) firstResPtr) >= startOfData);
    trav = prev = this->shm_ptr + *((size_t *) firstResPtr);
/*    sprintf(err_msg, "- %d -\n", *((size_t *) firstResPtr));
    err_msg_str = "";
    err_msg_str.append(err_msg);
    shm_log->write_message(err_msg_str, NULL); */
    nodeCnt++;
    em = 0;
/*    err_msg_str = "";
    err_msg_str2 = "";
    sprintf(err_msg, "%d ", *((int *) trav)); 
    err_msg_str.append(err_msg);
    sprintf(err_msg, "%d ", *((size_t *) (trav + sizeof(int)))); 
    err_msg_str2.append(err_msg); */
/* traverse list of results to find target position */
/*    sprintf(err_msg, "- %d -\n", *((size_t *) (trav + sizeof(int))));
    err_msg_str = "";
    err_msg_str.append(err_msg);
    shm_log->write_message(err_msg_str, NULL); */
    sprintf(err_msg, "- %d -\n", *((int *) (trav)));
    ...
    if (*((int *) trav) != data_id) {
        if ((trav + sizeof(int) + sizeof(size_t)) >= endOfSeg) {
            trav = this->shm_ptr + *((size_t *) startOfData);
        } else {
            trav = this->shm_ptr + *((size_t *) (trav + sizeof(int)));
        }
        while (trav != this->shm_ptr) {
                if (trav >= endOfSeg || trav < startOfData) {
                    shm_log->write_message("Error! Out of bounds in target search (shm_data_t::read_data)\n", NULL);
                    shmdt(this->shm_ptr);
                    mykey = ftok(SHM_DATA_FILENAME, 1);
                    this->intid = shmget(mykey, this->datasize, 0666 | IPC_CREAT);
/*                    this->attach_ptr = malloc(this->datasize);
                    if (this->attach_ptr == NULL) {
                        perror("malloc recover");
                        exit(-1);
                    }
                    free(this->attach_ptr); 
                    this->shm_ptr = (char *) shmat(this->intid, this->attach_ptr, SHM_RND); */
                    this->shm_ptr = (char *) shmat(this->intid, NULL, 0); 
                    if (this->shm_ptr < 0) {
                        perror("shmat recover");
                        exit(-1);
                    }
                    char * endOfSeg = this->shm_ptr + *((size_t *) this->shm_ptr);
                    char * nextResInt = this->shm_ptr + sizeof(size_t);
                    char * firstResPtr = this->shm_ptr + sizeof(size_t) + sizeof(int);
                    char * startOfData = this->shm_ptr + sizeof(size_t) + sizeof(int) + sizeof(size_t);
                    trav = this->shm_ptr + *((size_t *) firstResPtr);
                    nodeCnt = 0;
                    if (errseen) {
                        return(res);
                    }
                ...
                nodeCnt++;
                if (nodeCnt > this->datasize) {
                    shm_log->write_message("Error! Infinite loop in target search (shm_data_t::read_data)\n", NULL);
                    return(res);
                }
/*                sprintf(err_msg, "%d ", *((int *) trav)); 
                err_msg_str.append(err_msg);
...
/*                sprintf(err_msg, "- %d -\n", *((size_t *) (trav + sizeof(int))));
    ...
    target = trav;
/*    shm_log->write_message("Data id seq: ", NULL);
    shm_log->write_message(err_msg_str, NULL);
    shm_log->write_message("\n", NULL);
    shm_log->write_message("Offset seq: ", NULL);
    shm_log->write_message(err_msg_str2, NULL);
    shm_log->write_message("\n", NULL); */
    if (target == this->shm_ptr) {
...
/* BOUNDARY CHECK: to see if target (int) value would cross end of segment */
    if ((target + sizeof(int)) >= endOfSeg) {
        /* Set target to beginning of data portion of segment */
        target = (char *) startOfData;
    }
    /* BOUNDARY CHECK: to see if target (size_t *) value would cross end of segment */
    if ((target + sizeof(int) + sizeof(size_t)) >= endOfSeg) {
        /* Set target to beginning of data portion of segment */
        target = (char *) startOfData;
    } else {
        target += sizeof(int);
    ...
    endOfData = *((size_t *) target);
/* Read result vector char by char from segment */
    target += sizeof(size_t);
assert(target < endOfSeg);
assert(target >= startOfData);

    i = j = 0;
    ib = jb = -1; 
    fieldstr = "";
    while (target != (this->shm_ptr + endOfData)) {
        while (target != (this->shm_ptr + endOfData)) {
            while (target != (this->shm_ptr + endOfData)) {
                if (*target == '\0') {
                    break;
...
/* BOUNDARY CHECK: to see if target (char) value would cross end of segment */
               if ((target + sizeof(char)) >= endOfSeg) {
                  /* Set target to beginning of data portion of segment */
                     target = (char *) startOfData;
               } else {
                   target += sizeof(char);
           ...
           res[i][j] = fieldstr;
           fieldstr = "";
           if (target == (this->shm_ptr + endOfData)) {
               break;
           }
/* BOUNDARY CHECK: to see if target (char) value would cross end of segment */
           if ((target + sizeof(char)) >= endOfSeg) {
...
mysqlpool::mysqlpool(long max_conn, size_t data_size, size_t log_size, int debug) {
    ...
    this->shm_data = new shm_data_t(data_size);
...
vector<vector<string> > mysqlpool::run_query(string host, string user, string pass, string dbname, int port, string query) {
    ...
    *((int *) ((this->shm_data)->get_shm_ptr() + sizeof(size_t))) = ((data_id >= (10000000 - 1)) ? 0:(data_id + 1));
    ...
    res = (this->shm_data)->read_data(data_id, this->shm_log);
    delete this->msg;
    return(res);
}
